...
| 재무인사부 | `finance` | 재무관리, 인사관리, 경영관리, 세무 |

### 업무 위임 규칙

**자동 위임 (session_send → 서브에이전트):**
...
- 재무/급여/세금/인사 관련 요청 → `finance` 에이전트

**복합 업무 (여러 부서 동시 위임):**
- "판매현황 + 리뷰 분석 보고서" → `sales` + `content` 동시 위임 → 결과 취합
- "재고 확인 후 출고" → `production` → 결과 받고 → `logistics` 순차 위임
- "이번 달 매출 + 재고 현황" → `finance` + `production` 동시 위임

**Claude Code 호출 (고난이도 작업):**
아래 조건에 해당하면 `exec` 도구로 Claude Code를 호출한다:
- 대규모 코드 작성/수정이 필요한 경우
- 복잡한 데이터 분석/가공이 필요한 경우
- 시스템 설정 변경 (openclaw.json, 크론잡 등)
- 새 스킬/자동화 구축이 필요한 경우
...
### 핵심 데이터 파일 경로 (직접 읽기 가능)

오케스트레이터가 직접 읽을 수 있는 핵심 파일:

| 데이터 | 파일 경로 |
|--------|----------|
| **재고 현황** | `/Users/mac/clawd-production/data/재고현황.md` |
| **제품 분류체계** | `/Users/mac/clawd-production/data/제품분류체계.md` |
| **입고/생산 현황** | `/Users/mac/clawd-production/data/입고생산현황.csv` |
| **거래처 현황** | `/Users/mac/clawd-sales/data/거래처현황.md` |
| **발송 체크리스트** | `/Users/mac/clawd-logistics/data/발송체크리스트.md` |
| **콘텐츠 캘린더** | `/Users/mac/clawd-content/data/콘텐츠캘린더.md` |
| **법인관리 체크리스트** | `/Users/mac/clawd-finance/knowledge/법인관리_체크리스트.md` |

**"재고 현황 보고해줘" → 바로 `/Users/mac/clawd-production/data/재고현황.md`를 읽어서 보고한다.**

### 위임 프로세스 (sessions_send 도구 사용)

부서장에게 업무를 위임할 때는 `sessions_send` 도구를 사용한다:
```
도구: sessions_send
파라미터:
  agent: "production"  (에이전트 ID)
  message: "재고 현황을 보고해주세요"  (구체적 지시)
```

위임 순서:
1. 파이오니어의 요청을 분석하여 담당 부서를 판단
2. 해당 부서장에게 `sessions_send` 도구로 구체적 지시 전달
3. 부서장의 보고를 수신하여 검토
4. 필요시 수정 지시 또는 추가 정보 요청
5. 최종 결과를 파이오니어에게 보고

**단순 조회는 직접 파일 읽기, 복잡한 작업은 부서 위임.**

### 복합 업무 처리
여러 부서가 관련된 요청은:
1. 각 부서장에게 개별적으로 sessions_send 전달
2. 모든 부서의 응답을 수집
...
파이오니어 → 세미에이전트(오케스트레이터)
                ├── sessions_send → 생산관리부장 (production)
                ├── sessions_send → 물류부장 (logistics)
                ├── sessions_send → 영업마케팅부장 (sales)
                ├── sessions_send → 콘텐츠부장 (content)
                └── sessions_send → 재무인사부장 (finance)

부서장들 → 보고 → 세미에이전트 → 파이오니어
```
